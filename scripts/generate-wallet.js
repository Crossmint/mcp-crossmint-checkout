...
async function main() {
  try {
    ...
    console.log('\nCreating wallet...');
    const walletResponse = await createCrossmintWallet(process.env.RECIPIENT_EMAIL);
    ...
    envContent = envContent
      .split('\n')
      .filter(line => !line.startsWith('CROSSMINT_WALLET_ADDRESS='))
      .join('\n')
      .trim();

    // Add new wallet variables
    const newEnvContent = `${envContent}\n\nCROSSMINT_WALLET_ADDRESS=${walletResponse.address}\n`;
    await fs.promises.writeFile('.env', newEnvContent);
    console.log('\nWallet address has been updated in .env file');
